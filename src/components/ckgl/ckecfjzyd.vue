	...
	<div class='ckecfjzyd'>
		...
		<div class="but-box">
			<ul>
				<li>
					<div class="boxtab">
						<router-link to="pdaecfjzyck" class="watch">
							<div class="curbox ck">查看</div>
							<span style="display: none">二次分拣作业单</span>
						</router-link>
					</div>
					<span class="gun"></span>
				</li>
		...
		<div class="searchnav">
			...
			<el-form :inline="true"  class="demo-form-inline form1">
				...
				<el-col :span="2">
					...
					<el-col :span="4">
						<div class="grid-content ">
							<el-button type="primary" size="small" @click.native='seach' class='cx'>
		...
		<!--搜索框完-->
		<!--表格-->
		<el-table :data="tableData3" @row-click="rowClick" @row-dblclick="dbRow" highlight-current-row height="640" border style="width: 100%" >
			<el-table-column type="selection" prop="" label="选定框" width="50" >
			</el-table-column>
			<el-table-column prop="PickOperateID" label="分拣作业单号" width="auto">
			</el-table-column>
			<el-table-column prop="BillID" label="单据编号" width="auto">
			</el-table-column>
			<el-table-column prop="Date" label="日期" width="auto">
			</el-table-column>
			<el-table-column prop="PickQty" label="数量" width="auto">
			</el-table-column>
			<el-table-column prop="CareateDate" label="创建日期" width="auto">
			</el-table-column>
...
export default {
	...
	methods: {
			...
			watch() {
				var that = this;
				//跳转出库订单查看
				that.$router.replace({ path: '/pdaecfjzyck'}) 
				// 定义标签的名字
				let txt = "二次分拣作业单"
				// 取得标签内容
			...
			rowClick(row, event, column){
				...
				localStorage.setItem("ecfjzyBill", row.PickID);
			},
			// 行双击
			dbRow(row,event) {
				// 保存当前的订单号
		...
		seach(){
			...
			this.ajax({
				...
				data:{
					...
					method:'CPHH.PC.PDAModule.SecondPickOperate.GetList'
				},
				success:function(data){
					var obj = eval('(' + data + ')');
					var datas = JSON.parse(obj.Datas);
					var data = datas.Data;
					that.tableData3 = data; //单os
				}
			})
		},
		onSubmit() {
...
.ckecfjzyd .el-form-item__content {
	margin-top: 6px;
	margin-left: 10px;
}

.ckecfjzyd .el-table th>.cell {
	line-height: normal;
}

.ckecfjzyd .el-table tr:nth-child(2) th {
	background: none;
}
...
.xg {
	background: url(../../../common/img/rk/xg.png) no-repeat;
	background-position: 5px 10px;
	background-size: 20px 20px;
}

.fpry {
	background: url(../../../common/img/rk/fenpei.jpg) no-repeat;
	background-position: 5px 10px;
	background-size: 20px 20px;
}
...
.jhzy {
	background: url(../../../common/img/rk/ckjh.png) no-repeat;
	background-position: 5px 10px;
	background-size: 20px 20px;
}
