<template>
	<!--HTML代码写这个标签里面-->
	<div class="ckgl" style="position:relative">
		<form class="page2">
			<!--第二页-->
			<!--<div class=>-->
			<!-- element步骤组件 -->
			<div class="select_box">
								...
								<div class="curbox cz">
									...
									<el-dropdown split-button @command="handleClick">
										...
										<el-dropdown-menu slot="dropdown">
											<el-dropdown-item command="lrfy">录入费用项</el-dropdown-item>
				...
				<el-row :gutter="10">
					...
					<el-col :span="3">
						<div class="grid-content  ">
							<!-- 配送中心列表 -->
							<el-select disabled v-model="peisong" id="pszx" @change="pschange" filterable placeholder="" class=" blueborder input_width">
					...
					<el-col :span="3">
						<div class="grid-content  ">
							<!-- 园区列表 -->
							<el-select v-model="yuanqu" placeholder="" id="yqlb" :disabled="pszx" no-data-text="23132" class=" blueborder input_width">
					...
					<el-col :span="3">
						<div class="grid-content  ">
							<el-input v-model="ddbh" disabled placeholder="" class="input_width blueborder yuandanhao"></el-input>
						</div>
					</el-col>
					<el-col :span="2">
						<div class="grid-content  ">
					...
					<el-col :span="3">
						<div class="grid-content  ">
							<div class="">
								<el-input v-model="ckdh" placeholder="" class="input_width blueborder yuandanhao"></el-input>
							</div>
						</div>
					</el-col>
				</el-row>
				<el-row :gutter="10">
					...
					<el-col :span="3">
						<div class="grid-content  ">
							<el-input class="input_width blueborder" disabled placeholder="" icon="search" v-model="hzf" @click="hzf_Click = true" id='hzf' ref='hzf'>
							</el-input>
						</div>
					</el-col>
					<el-col :span="2">
					...
					<el-col :span="3">
						<div class="grid-content  ">
							<el-input class="input_width blueborder" disabled icon="search" v-model="kehu" @click="kh_Click = true"></el-input>
						</div>
					</el-col>
					<el-col :span="3">
						<div class="grid-content  ">
					...
					<el-col :span="3">
						<div class="grid-content  ">
							<div id="ckrq">
								<el-date-picker class="input_width" v-model="ckrq" align="right" type="date" disabled :picker-options="pickerOptions1">
								</el-date-picker>
							</div>
				...
				<el-row :gutter="10">
					...
					<el-col :span="8">
						<div class="grid-content">
							<el-input v-model="shdz" disabled icon="search" class='input_width' @click='dizhi_Click=true'></el-input>
						</div>
					</el-col>
				...
				<el-row :gutter="10">
					...
					<el-col :span="3">
						<div class="grid-content  ">
							<div>
								<el-button type="primary" class='btnzz' @click="jgfs_Click=true">加工 </el-button>
							</div>
						</div>
					</el-col>
				</el-row>
			...
			<!--商品表格部分-->
			<el-table :data="splbs" border style="width:100%" id="spbg" class='spbg' min-height="300" max-height="310" v-loading.body="loading" @row-contextmenu="rowDelect" @cell-click="tbclick" @cell-dblclick="dbHandle" @current-change="dbchange" element-loading-text="玩命加载中">
				<el-table-column prop="index" type="index" class="index" label="序号" width="100" fixed sortable>
				</el-table-column>
				<el-table-column prop="ProCode" label="商品编码" width="110" class-name="Code bianji" v-model='spbm'>
					<!-- 可编辑写法start -->
				...
				<el-table-column prop="Desc" label="备注" class-name="bianji">
					<!-- 可编辑写法start -->
					<template scope="scope">
			...
			<ul class='heji_ul'>
				...
				<li>
					<span style="color:red">{{ isNaN(getBasicAmount) ? 0 : getBasicAmount }}</span>
				</li>
				<li>
					<span>辅助合计</span>
				</li>
				<li>
					<span style="color:red">{{ isNaN(getAuxiliaryNumber) ? 0 : getAuxiliaryNumber}}</span>
				</li>
				<li>
					<span>金额合计</span>
				</li>
				<li>
					<span style="color:red">{{ isNaN(getTotalPrice) ? 0 : getTotalPrice}}</span>
				</li>
			</ul>
			<!--第二表单-->
			<div class='form-2'>
				...
				<el-form :inline="true" v-model="formInline" class="demo-form-inline">
					<el-form-item label="采购员" class='fl'>
						<el-input v-model="caigouyuan" placeholder="" disabled class='fr input_width button_w'></el-input>
					</el-form-item>
			...
			<!--合作方弹窗 end-->
			<!--供应商-->
			<el-dialog title="选择供应商" class="gys" :visible.sync="kh_Click" size="tiny" :before-close="handleClose">
				...
				<span slot="footer" class="dialog-footer gys-footer" style=' top:480; left:730'>
					...
					<el-button type="primary" @click="checkgetkehudizhi(khInfo)">确 定</el-button>
				</span>
			</el-dialog>
			<!--供应商弹窗 end-->
...
<script>
	...
	export default {
		...
		mounted: function() {
			var getInfo = window.localStorage.getItem("ckdDataStorage");
			// console.log("得到的信息=》" ,getInfo)
			// var getInfo = 'XSCK003-161000001';
		  this.afterStorageID = getInfo;
			...
			$(window).on("keydown", function(e) {
				...
				// 2.如果是备注
				if(that.enterFlag == "Desc" && e.key == "Enter") {
			...
			// 打印
			$('html').on('click', '.ckgl .dy', function() {
				$('#spbg').printArea();
			});
			$('html').on('click', '.ckgl .el-breadcrumb-item .close', function() {
				$('.ckgl').hide();
      ...
      dbHandle:function(row, column, cell, event){
        let key = column.property+'Edit';
				...
				console.log(5555,this.enterFlag);
        setTimeout(()=>{
          try{
            $("input",cell).focus();
          }catch(err){
        ...
        for(let i=0; i<that.splbs.length; i++){
          ...
          var Pro = {
							...
							Desc: sp.Desc || "",
							BodyProperty1: sp.BodyProperty1,
							BodyProperty2: sp.BodyProperty2,
							BodyProperty3: sp.BodyProperty3,
							BodyProperty4: sp.BodyProperty4,
							IsFree: sp.IsFree,
							Batch: sp.Batch,
							Account1: sp.Account1,
							Account2: sp.Account2,
							Account3: sp.Account3,
							Account4: sp.Account4,
							Account5: sp.Account5,
							ProductionDate: sp.ProductionDate,
							EffectiveDate: sp.EffectiveDate,
							IsAppointBatch: sp.IsAppointBatch || 0,
							OrderEntryID: sp.OrderEntryID,
							OrderNum: sp.OrderNum,
							OrderID: sp.OrderID
					};
					Pros.push(Pro);
				}
				...
				jsons.Delivery = that.jhfss[that.jhfs-1].label; // 交货方式
				// jsons.DeliveryAddress = that.shdz; //送货地址
				jsons.Transport = that.ysfss[that.ysfs-1].label; // 运输方式
				...
				$.ajax({
					...
					data: {
						datas: jsons,
						method: "CPHH.PC.OutModule.Out.Edit",
						test: 'test'
					},
					timeout: 5000, //超时时间
        ...
        var that = this;
			...
			getshopInfo(StorageID) {
				...
				$.ajax({
					...
					data: {
						datas: params,
						method: "CPHH.PC.OutModule.Out.Look",
						// test:"test"
					},
					timeout: 5000, //超时时间
					success: function(data, textStatus, jqXHR) {
          	...
          	that.saveData = data; //设置备份
						...
						if(data[0].SubmitStatus === "提交" || data[0].SubmitStatus === 1){
						  that.judgeTJ = true
						} else {
							that.judgeTJ = false
						}
					  ...
					  that.sfsx = data[0].IsJM + '';
					  that.caigouyuan = data[0].SalesMan;
					  that.zhidanren = data[0].SalesMan;
			...
			getkehuData() {
				...
				that.baseAjax("suppliergetlist", {
					...
					var datas = JSON.parse(obj.Datas);
					var data = datas.Data;
					that.kehu = data[0].Name;
					that.khid = data[0].InterID;
				});
			},
				// 客户信息
			getkehuInfo (data) {
       ...
       this.kehu = data.Name;
        ...
        for(let i = 0; i < val.length; i++) {
					val[i].ProName = val[i].Name;
					val[i].ProCode = val[i].Code;
					val[i].ProModel = val[i].Model;
				} 
				val.forEach((item, index) => {
			...
			getCWInfo(val) {
				...
				this.$set(this.splbs[this.lookShopNum],'StockName',val.Name);
          ...
          var getStockID = this.StockData[0].StockID;
            ...
            this.splbs[i].StockID = getStockID;
            this.splbs[i].StockName = getStockName;
					}
          ...
          this.ckzy_Click = true;
			...
			getTotal() {
				...
				for(let i = 0; i < this.splbs.length; i++) {
					basicAmount += parseFloat(this.splbs[i].Num);
					auxiliaryNumber += parseFloat(this.splbs[i].SupNum);
					totalPrice += parseFloat(this.splbs[i].Total);
				}
        ...
        this.StockData[0].StockName = row.StockName;
				...
				//单击表格增加颜色
				if(jname == "Price" || jname == "Num" || jname == "SupNum" || jname == "SupPrice" || jname == "Desc" || jname == "Code" || jname == "StockName") {
            ...
            StaffID: that.WarehouseWork[i].InterID,
				...
				for ( let i = 0; i < that.FeeTacticsParam.length; i++) {
					var FeeTacticsParams = {
						FeeModuleID: parseInt(that.FeeTacticsParam[i].FeeModuleID),
				...
				var jsons = {
					...
					TransportCmpID: 0,
					TransportCmpName: '',
					HandlingTeamID: that.zxdlist.InterID,
					HandlingTeamName: that.zxdlist.Name,
