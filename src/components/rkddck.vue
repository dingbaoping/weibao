		...
		<form class="page2">
			...
			<div class="select_box">
				...
				<el-row :gutter="10" type="flex">
					...
					<el-col :xs="8" :sm="8" :md="8" :lg="8">
						<div class="grid-content ">
							...
							<el-input v-model="dz" placeholder="" icon="search" class='fr blueborder dizhi input_width' @click='dizhi_Click=true'></el-input>
						</div>
					</el-col>
				...
				<el-row :gutter="10" type="flex">
					...
					<el-col :xs="8" :sm="8" :md="8" :lg="8">
						<div class="grid-content  ">
							<el-input v-model="bz" disabled placeholder="" class='fr blueborder dizhi input_width'></el-input>
						</div>
					</el-col>
				</el-row>
			</div>
			...
			<!--商品表格部分-->
			<el-table :data="splbs" border style="width:100%" id="spbg" class='spbg' min-height="300" max-height="310" v-loading.body="loading" @row-contextmenu="rowDelect" @cell-click="tbclick" @cell-dblclick="dbrow" @current-change="dbchange" element-loading-text="玩命加载中">
				...
				<el-table-column prop="IsFree" label="赠品标志" width="104" class-name="bianji">
					<template scope="scope">
						<el-select v-model="splbs[scope.$index].IsFree" disabled placeholder="请选择">
...
<script>
	...
	export default {
		...
		mounted: function() {
			var getInfo = window.localStorage.getItem("dataStorage");
		  ...
		  this.afterStorageID = getInfo;
			...
			for(var i = 0; i < 101; i++) {
				jsons.push({
					...
					nomber: '',
					price: parseInt(12.02 + i)
				});
			}
			that.tableData = jsons;
			// 回车直接删除商品
			$(window).on("keydown", function(e) {
				...
				if(that.enterFlag == "Remarks" && e.key == "Enter") {
					// if(!that.splbs[that.splbs.length].Code){
					console.log(that.splbs[that.splbs.length - 1].BarCode);
					that.splbs[that.splbs.length - 1].BarCode && that.splbs.push({
						Code: ''
					});
					// }
					// console.log('enter',that.splbs);
				}
			})
			...
			$("#save").click(function() {
				if(that.hzf == '') {
					$('html').on('load', 'alert2 .change', function() {
						$('alert2 .change').html('合作方');
					})
					that.alert2_Click = true;

				} else if(that.gys == '') {
					$('html').on('load', 'alert2 .change', function() {
						$('alert2 .change').html('供应商');
					})
					that.alert2_Click = true;
				} else if(that.spbm == '') {
					that.alert1_Click = tru;
				} else {
					var form1_data = [];
					form1_data.push(that.peisong);
					form1_data.push(that.yuanqu);
					form1_data.push(that.ddbh);
					form1_data.push(that.hzf);
					form1_data.push(that.gys);
					form1_data.push(that.rklx);
					form1_data.push(that.jhfs);
		...
		methods: {
			getshopInfo(StorageID) {
				...
				$.ajax({
					...
					data: {
						datas: params,
						method: "CPHH.PC.StorageModule.StorageOrder.Look"
					},
					timeout: 5000, //超时时间
					success: function(data, textStatus, jqXHR) {
						...
						if (data[0].Transport === "汽运") {
              that.ysfs = '0';
						} else if (data[0].Transport === "铁运") {
              that.ysfs = '1';
						} else if (data[0].Transport === "海运") {
              that.ysfs = '2';
						} else {
              that.ysfs = '3';
						}
						...
						for(let i = 0; i < that.splbs.length; i++) {
							console.log(that.splbs[i].ProName)
							if(that.splbs[i].ProName) {
			...
			dbrow: function(row, column, cell, event) {
				...
				if(jname != "Price" && jname != "Num" && jname != "SupNum" && jname != "SupPrice" && jname != "Remarks" && jname != "Code" || true	) {
					//如果当前列不允许编辑.则跳出
					return false;
				}
				var that = this;
      ...
      //       that.splbs[i].IsFree = '0'
			...
			// 		data: {
			// 			datas: jsons,
			// 			method: "CPHH.PC.StorageModule.StorageOrder.Edit"
			// 		},
			// 		timeout: 5000, //超时时间
			// 		success: function(data, textStatus, jqXHR) {
