<template>
	<div class="ckddxg" style="position:relative">
		<form class="page2">
			<!--第二页-->
			<!--出库单-->
			<div class="but-box">
				<ul>
					<li>
						<div class="boxtab">
							<div class="curbox bcsh" @click="saveOrder">
								<span>保存并审核</span>
							</div>
						</div>
						<span class="gun"></span>
			...
			<el-dialog title="商品资料" class="spzl" :visible.sync="spzl_Click" size="tiny" :before-close="handleClose">
				<spzl :spstr="spStr" :DCID="saveData[0].DCID" :sphzf="hzf" :ParkID="saveData[0].ParkID" :sphzfID="saveData[0].ParID" @proSelect="setProduct">
			...
			<el-dialog style="text-align:center" title="系统提示" :visible.sync="bcsb_Click" size="tiny" :before-close="handleClose" class='bcsb'> 	
				...
				<!-- 保存信息 -->
				<p>{{ bcxx }}</p>
				<el-button type="primary" style="display: block; margin: 0 auto;" class='dialog_ok' @click.native="bcsb_Click = false">确 定</el-button>
				</span>
			</el-dialog> 
...
<style rel="stylesheet/stylus" scoped>
	...
	.ckddxg .el-input-number {
    width: 100%;
	}
	
	.ckddxg .el-date-editor.el-input {
		width: 100%;
	}
	.el-row {
		margin-bottom: 2px;
			...
			getshopInfo (OutID){
				...
				$.ajax({
					...
					success: function(data, textStatus, jqXHR) {
						...
						that.getTransports(data[0].DCID); 
						...
						that.chuangjianfs = data[0].CreateWay; //创建方式
          ...
          var Pro = {
				...
				var  jsons = that.saveData[0]; //从备份处获取
				      if(that.shifou == "是") {
							...
							if(that.sfps === true) {
								jsons.NeedDeliveryDateMark = 1;
								jsons.NeedDeliveryDate = $('#xpssj input').val();
							} else {
								jsons.NeedDeliveryDateMark = 0;
								jsons.NeedDeliveryDate = that.saveData[0].NeedDeliveryDate;
							}
							// 更新修改值
							jsons.OutID = that.afterStorageID; // 订单编号
							jsons.Level = that.num; //订单级别
							jsons.Pro = Pros; //商品
							jsons.UserID = userid; //商品
							jsons.UserName = userName; //商品
							jsons.ParkID = that.yuanqu; //园区id
							jsons.CusAreaID = that.khqy; // 客户区域ID
							jsons.CusAreaName = $(".khqy input").val(); // 客户名称
							...
							jsons.IsJM = that.shifou; //源单号
							jsons.ContactMobile = that.lxdh;
