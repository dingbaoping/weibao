		...
		methods: {
          ...
          let $parent = $cell.parent();
			...
			getRow(row, event) {
				...
				this.delIndex = $(event.target).parent().parent().parent().index(); //获取等坐标
				console.log("行坐标 --  " + $(event.target).parent().parent().parent().index())
			},
			// 行删除
			rowDelect(row, event) {
			...
			addOrder() {
				var that = this;
				if (that.jhfs == '1' && that.dz == '') {
          that.bcsb_Click = true;
					that.infoHint = '交货方式为：派车, 请填入地址';
					return
				}
