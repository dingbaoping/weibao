  ...
  <div class="rkd" style="position:relative">
    ...
    <form class="page2">
      ...
      @cell-dblclick="dbHandle" class='spbg' height="305" v-loading.body="state.loading" 
        ...
        <el-table-column prop="Code" label="商品编码" width="110" class-name="bianji">
      ...
      <!-- 仓位数据弹窗-->
      <el-dialog title="请选择批号" class="cwzl" :visible.sync="state.cwzl_Click" size="tiny" :before-close="handleClose">
        
        <cwzl :yqID="psData.yuanqu" @sendCW="acceptCW"></cwzl>
        <span slot="footer" class="dialog-footer">
      ...
      that.getTotal();
			...
			$(window).on("keydown", function(e) {
				...
				// 2.如果是备注
					...
					that.splbs[that.splbs.length - 1].Name && that.splbs.push({
						Code: ''
					});
				}
				// 3.如果是编码
    ...
    methods: {
      ...
      hatchData(data) {
        ...
        for(let i = 0; i < that.splbs.length; i++) {
          that.splbs[i].Code = data.Pro[i].ProCode;
          that.splbs[i].Name = data.Pro[i].ProName;
          that.splbs[i].Model = data.Pro[i].ProModel;
          that.splbs[i].InterID = data.Pro[i].ProID;
          ...
          that.splbs[i].Remarks = data.Pro[i].Desc;
      ...
      // 仓位点击前判断是否有物料
      lookCW () {
        if( this.splbs[this.lookShopNum].Name) {
          ...
          that.splbs.forEach((item,index,data)=>{
            console.log(item);
              ...
              let prod = {
                ...
                Desc:item.Remarks,//备注
                BodyProperty1:'',//自定义1
                BodyProperty2:'',//自定义2
                BodyProperty3:'',//自定义3
                BodyProperty4:'',//自定义4
                IsFree:item.IsFree,//是否赠品
                Batch:item.BatchManage,//批次
                Account3:'',//库存描述
                Account4:'',//库存属性1
                Account5:'',//库存属性2
                ...
                OrderNum: item.OrderNum || 0,//订单数量
                OrderID: item.OrderID ||'123',//订单号
      ...
      checkgetgysdizhi(info) {
        // console.log(info);
        this.gys = info.Name
        this.psData.gysid = info.InterID;
