<template>
	...
	<!--表格-->
	<el-table class="table" :data="splbs" height="645" @row-click="rowClick" @row-dblclick="dbRow" @header-click="headerc" highlight-current-row border style="width: 100%">
	...
	<div class='tongji'>
		<el-tag class='tag1'>上架数量<span> {{ SJNum }} </span></el-tag>
	</div>
	</div>
...
<script>
	export default {
		...
		methods: {
			...
			dbRow(row,event) {
				// 保存当前的订单号
				localStorage.setItem("sjzyPickID", row.PickID);
				this.watch(); //跳查看页
			},
			// 行单击保存单号
			...
			delect() {
				...
				if(this.operateID == '') {
					...
					var jsons = {
						ShelvesID: this.operateID,//	作业单号	string
						...
						$.ajax({
							...
							success: function(data, textStatus, jqXHR) {
								...
								if(obj.StateCode == 100) {
									that.openAlert("单据删除成功",'系统提示', 'success')
									that.search();
								} else {
			...
			search (){
				...
				$.ajax({
					...
					success: function(data, textStatus, jqXHR) {
						...
						// 保存默认单号
						if(data.length > 0) {
							this.operateID = data[0].ShelvesID;
						}
						console.log(data)
						that.splbs = data; //单据
						that.getTotal(data)
			...
			getTotal (data) {
				...
				for(let i = 0; i < data.length; i++) {
					sjNum += data[i].ShelvesQty;
				}
				that.SJNum = sjNum.toFixed(4);
			},
			onSubmit() {
				console.log('submit!');
			},
